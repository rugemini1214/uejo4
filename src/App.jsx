...
const parseMaterialString = (str) => {
    ...
    return str.split('/').map(part => {
        ...
        return match 
            ? { name: match[1].trim(), percent: match[2] || '' }
            : { name: part.trim(), percent: '' };
    });
};
...
const serializeMaterialRows = (rows) => {
    return rows
        .filter(r => r.name)
        .map(r => `${r.name.toUpperCase()}${r.percent ? ' ' + r.percent + '%' : ''}`)
        .join(' / ');
};
...
export default function App() {
  ...
  const EditPage = () => (
      <div className="bg-white min-h-full pb-20 animate-fade-in font-mono">
          <div className="px-6 py-5 border-b border-gray-50 flex justify-between items-center sticky top-0 bg-white z-40">
              ...
              <div className="space-y-3">
                  ...
                  <div className={`pt-2 transition-opacity ${customConditions.targetColor ? 'opacity-30' : ''}`}>
                      <input 
                        ...
                        className="w-full border-b border-gray-200 py-2 text-sm font-mono outline-none uppercase placeholder-gray-300" 
                        value={customConditions.targetId} 
                        onChange={e => setCustomConditions(prev => ({...prev, targetId: e.target.value.toUpperCase(), targetColor: ''}))} 
                        onFocus={() => setCustomConditions(prev => ({...prev, targetColor: ''}))}
                      />
                  </div>
              </div>
          </div>
        ...
        {view !== 'edit' && (
            <>
                <div className="flex-1 overflow-y-auto scrollbar-hide">
                    {view === 'wardrobe' && <WardrobePage />}
                    {view === 'organize' && <OrganizePage />}
                    {view === 'outfit' && <OutfitPage />}
